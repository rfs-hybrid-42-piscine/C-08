...
2. **Testing:**
   Use the master **[`tester.c`](../tester.c)** file provided in the root `C-08` directory. You must pass the `-D EX04` flag to the compiler to selectively activate the test for this specific exercise! Since this requires `ft_stock_str.h`, you must compile with the `-I .` flag so the compiler knows to check the current directory for the header!
   ```bash
